...
export const InstructorCard: React.FC<InstructorCardProps> = ({ instructor }) => {
  ...
  return (
    <Card className="overflow-hidden group transition-all hover:shadow-lg hover:-translate-y-1 duration-300">
      ...
      <CardContent className="p-5">
        <Link to={`/instructor/${id}`} className="block">
          <h3 className="text-xl font-bold mb-1 group-hover:text-schoolier-blue transition-colors">
            {name}
          </h3>
        </Link>
        ...
        <div className="flex items-center justify-between text-sm pt-3 border-t">
          <div className="flex items-center gap-1">
            <Users className="h-4 w-4 text-schoolier-blue" />
            <span>{students.toLocaleString()} étudiants</span>
          </div>
          <div className="flex items-center gap-1">
            <BookOpen className="h-4 w-4 text-schoolier-teal" />
